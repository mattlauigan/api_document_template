  ...
  return (
    <div className="border border-gray-200 rounded-lg p-6 mb-6">
      <div className="flex items-center space-x-4 mb-4">
        <span className={`px-3 py-1 rounded-full text-sm font-medium ${methodColors[endpoint.method]}`}>
          {endpoint.method}
        </span>
        <code className="text-sm bg-gray-100 px-3 py-1 rounded">{endpoint.path}</code>
      </div>
        ...
        {endpoint.responses.map((response) => (
          <div key={response.status} className="mb-4 last:mb-0">
            <div className="flex items-center space-x-2 mb-2">
              <span className="text-sm font-medium text-gray-900">Status: {response.status}</span>
